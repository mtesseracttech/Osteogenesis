...
namespace Osteogenesis
{
    public class GaussArea
    {
        ...
        private float GaussAreaVertex(Vector3 vertex, Mesh mesh)
        {
            ...
            var angles = new List<float>(projectedNormals.Count);
            foreach (var projectedNormal in projectedNormals)
            {
                angles.Add(Vector3.SignedAngle(rotationStart, projectedNormal, pivotNormal));
            }
            
            Debug.Assert(angles.Count == relatedNormals.Count);
            ...
            var angleVectorPairs = new List<(float, Vector3)>(angles.Count);
            for (int i = 0; i < angles.Count; i++)
            {
                angleVectorPairs.Add((angles[i], relatedNormals[i]));
            }
